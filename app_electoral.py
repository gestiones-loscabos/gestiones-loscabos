...
with st.sidebar:
    ...
    msg_hub = "Hola, te comparto el enlace operativo: " + str(link_seleccionado)
    link_w_hub = "https://wa.me/52" + str(celular_hub) + "?text=" + urllib.parse.quote(msg_hub)
    st.markdown('<a href="' + link_w_hub + '" target="_blank" style="display: block; text-align: center; background-color: #25D366; color: white; padding: 6px; border-radius: 6px; text-decoration: none; font-size: 12px; font-weight: bold;">📲 Enviar Link por WhatsApp</a>', unsafe_allow_html=True)
...
if st.session_state.seccion_activa == "TABLERO":
    ...
    if st.session_state.ver_modal_brigadistas:
        ...
        with st.form("form_alta_brigadista_desplegable"):
            ...
            link_w_brig = "https://wa.me/52" + str(b_cel) + "?text=" + urllib.parse.quote("Hola " + str(b_nom) + ", tu tarea de campo está lista.") if b_cel else "#"
            st.markdown('<a href="' + link_w_brig + '" target="_blank" style="display: block; text-align: center; background-color: #25D366; color: white; padding: 6px; border-radius: 6px; text-decoration: none; font-size: 12px; font-weight: bold; margin-bottom: 10px;">📲 Enviar Tarea por WhatsApp</a>', unsafe_allow_html=True)
...
# ==========================================
    ...
    with tab_3:
        ...
        msg_panico = "¡EMERGENCIA EN CASILLA! Incidente reportado: " + str(incidencia_reporte)
        link_panico = "https://wa.me/52" + str(cel_emergencia) + "?text=" + urllib.parse.quote(msg_panico)
        st.markdown('<a href="' + link_panico + '" target="_blank" style="display: block; text-align: center; background-color: #dc2626; color: white; padding: 12px; border-radius: 6px; text-decoration: none; font-weight: bold; font-size: 16px; margin-top: 10px;">🚨 ACTIVAR ALERTA URGENTE POR WHATSAPP</a>', unsafe_allow_html=True)
        st.markdown("</div>", unsafe_allow_html=True)
...
# ==========================================
    ...
    with st.form("form_difusion_masiva"):
        ...
        link_w_dif = "https://wa.me/52" + str(cel_prueba_dif) + "?text=" + urllib.parse.quote(texto_difusion)
        st.markdown('<a href="' + link_w_dif + '" target="_blank" style="display: block; text-align: center; background-color: #25D366; color: white; padding: 8px; border-radius: 6px; text-decoration: none; font-weight: bold; margin-bottom: 10px;">📲 Enviar Mensaje de Prueba por WhatsApp</a>', unsafe_allow_html=True)
